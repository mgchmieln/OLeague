...
import { FantasyFootballApi } from '../api/FFApi';
...
class UrlEntryModal extends Component {
  ...
  async parseLocalStorage() {
    ...
    const leagueDatas = await Promise.all(
      leagueKeys.map(async key => {
        ...
        return { leagueId, seasonId, name: leagueData.name };
      })
    );
            ...
            <Dropdown text="Recent Searches" icon="find" floating labeled button className="icon">
              <Dropdown.Menu>
                ...
                {this.state.recentRankings.map(recentRanking => {
                  return (
                    <Dropdown.Item
                      key={recentRanking.leagueId}
                      onClick={() => this.onRecentRankingSelection(recentRanking)}
                    >
                      {recentRanking.name}
                    </Dropdown.Item>
                  );
